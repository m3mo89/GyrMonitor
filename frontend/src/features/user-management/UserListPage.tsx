...
export function UserListPage() {
  ...
  function handleCreateSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!name.trim() || !email.trim() || !password) {
      setFormError('Nombre, correo y contrasena son obligatorios.');
      return;
    }

    if (password.length < minimumPasswordLength) {
      setFormError(`La contrasena debe tener al menos ${minimumPasswordLength} caracteres.`);
      return;
    }
  ...
  function handleResetSubmit(event: FormEvent<HTMLFormElement>, userId: string) {
    ...
    if (newPassword.length < minimumPasswordLength) {
      setResetError(`La contrasena debe tener al menos ${minimumPasswordLength} caracteres.`);
      return;
    }
