...
Define the cattle reference foundation for GyrMonitor, including MVP cattle records, protected cattle list/detail access, a cattle history contract placeholder, seed data, and frontend cattle list/detail workflows.
...
The backend SHALL reserve protected `GET /api/v1/cattle/{id}/events` behavior as the cattle history contract placeholder for later activity-event integration.

#### Scenario: Existing cattle history route is callable
- **WHEN** an authenticated `ADMIN` or `RESEARCHER` requests history for an existing cattle UUID
- **THEN** the API returns the documented placeholder response shape without inventing activity-event, alert, or observation data
