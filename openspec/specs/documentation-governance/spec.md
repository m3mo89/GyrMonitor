...
Ensure that project documentation — the `knowledge-base/`, root-level status documents, roadmap/release-notes tracking, and versioned academic deliverables in `docs/` — stays synchronized with the actual archived-change history, so the docs describe the system's current implemented state rather than a stale or historical snapshot.
