# MAUI Client Architecture Specification

## Purpose

TBD - Define the Clean Architecture and Screaming Architecture boundaries that `.NET MAUI` mobile and desktop client feature code must follow, so client-side layering, dependency direction, and MVVM composition stay consistent with the backend's architectural discipline.
